...
<div class="panel page-head"><h1>Match Researchers to a Funding Call</h1><form method="get" class="filters-grid matching-grid"><input type="hidden" name="page" value="matching"><select name="funding_call_id"><option value="">-- choose a funding call --</option><?php foreach($fundingCalls as $fc): ?><option value="<?= (int)$fc['id'] ?>" <?= (int)$fc['id']===$selectedId?'selected':'' ?>><?= h($fc['title']) ?></option><?php endforeach; ?></select><button class="primary-btn" type="submit">Find Matches</button><?php if($selected): ?><form method="post" style="display:inline"><input type="hidden" name="action" value="compute_matches"><input type="hidden" name="funding_call_id" value="<?= $selectedId ?>"><?= csrf_input() ?><button class="ghost-btn" type="submit">⚡ Compute AI Matches</button></form><?php endif; ?></form><div class="muted small">Tip: Matching score is explainable: topics = 2 pts, geography = 1 pt.<?php if($selected && !$hasAiScores): ?> No AI scores computed yet — click ⚡ above to start.<?php endif; ?></div></div>
